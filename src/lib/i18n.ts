...
if (!i18n.isInitialized) {
  ...
  if (typeof window !== 'undefined') {
    const log13 = {location:'i18n.ts:init',message:'i18n init start',data:{time:Date.now(),detectedLang:detectedLang},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'H2'};
  ...
  initPromise = i18n
    ...
    .then(() => {
      ...
      if (i18n.language !== detectedLang) {
        // #region agent log
        if (typeof window !== 'undefined') {
          const log15 = {location:'i18n.ts:init',message:'Language mismatch in init, fixing',data:{time:Date.now(),from:i18n.language,to:detectedLang},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'H3'};
        ...
        return i18n.changeLanguage(detectedLang).then(() => {
          // #region agent log
          if (typeof window !== 'undefined') {
            const log16 = {location:'i18n.ts:init',message:'Language fixed after init',data:{time:Date.now(),newLang:i18n.language,hasResourceBundle:i18n.hasResourceBundle(i18n.language,'translation')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'H3'};
    ...
    .catch((error) => {
      // #region agent log
      if (typeof window !== 'undefined') {
        const log17 = {location:'i18n.ts:init',message:'i18n init error',data:{time:Date.now(),error:String(error)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'H2'};
  ...
  // #region agent log
  if (typeof window !== 'undefined') {
    const log18 = {location:'i18n.ts:init',message:'i18n already initialized',data:{time:Date.now(),i18nLanguage:i18n.language,detectedLang:detectedLang},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'H2'};
